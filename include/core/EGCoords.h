...
// throughout the library.  Most importantly, EG defines a framework for
...
//
//  (x, y, z)
//    Direction vector (EGPoint).  Direction vectors are not necessarily unit
...
//
//  (lat, lng)
//    Latitude and longitude (EGLatLng).  Latitudes must be between -90 and
...
#ifndef EG_EGCOORDS_H_
#define EG_EGCOORDS_H_
...
#include "core/r2.h"
#include "core/EGCoords_internal.h"
#include "core/EGPoint.h"
#include "util/math/mathutil.h"

// EG is a namespace for constants and simple utility functions that are used
// throughout the EG library.  The name "EG" is derived from the mathematical
...
// constant is defined here so that the EG::Metric class and the conversion
// functions below can be implemented without including EGcell_id.h.  Please
// see EGcell_id.h for other useful constants and conversion functions.
...
const int kLimitIJ = 1 << kMaxCellLevel;  // == EGCellId::kMaxSize
...
EGPoint FaceUVtoXYZ(int face, double u, double v);
EGPoint FaceUVtoXYZ(int face, const R2Point& uv);
...
bool FaceXYZtoUV(int face, const EGPoint& p,
                        double* pu, double* pv);
bool FaceXYZtoUV(int face, const EGPoint& p, R2Point* puv);
...
void ValidFaceXYZtoUV(int face, const EGPoint& p,
                             double* pu, double* pv);
void ValidFaceXYZtoUV(int face, const EGPoint& p, R2Point* puv);
...
//
// This data was produced using EGcell_test and EGcell_id_test.

#define EG_LINEAR_PROJECTION    0
#define EG_TAN_PROJECTION       1
#define EG_QUADRATIC_PROJECTION 2

#define EG_PROJECTION EG_QUADRATIC_PROJECTION

#if EG_PROJECTION == EG_LINEAR_PROJECTION
...
#elif EG_PROJECTION == EG_TAN_PROJECTION
...
#elif EG_PROJECTION == EG_QUADRATIC_PROJECTION
...
#error Unknown value for EG_PROJECTION

#endif

inline double IJtoSTMin(int i) {
  EG_DCHECK(i >= 0 && i <= kLimitIJ);
...
inline EGPoint FaceUVtoXYZ(int face, double u, double v) {
  switch (face) {
    case 0:  return EGPoint( 1,  u,  v);
    case 1:  return EGPoint(-u,  1,  v);
    case 2:  return EGPoint(-u, -v,  1);
    case 3:  return EGPoint(-1, -v, -u);
    case 4:  return EGPoint( v, -1, -u);
    default: return EGPoint( v,  u, -1);
  }
}

inline EGPoint FaceUVtoXYZ(int face, const R2Point& uv) {
  return FaceUVtoXYZ(face, uv[0], uv[1]);
}

inline void ValidFaceXYZtoUV(int face, const EGPoint& p,
                             double* pu, double* pv) {
  EG_DCHECK_GT(p.DotProd(GetNorm(face)), 0);
  switch (face) {
...
inline void ValidFaceXYZtoUV(int face, const EGPoint& p, R2Point* puv) {
  ValidFaceXYZtoUV(face, p, &(*puv)[0], &(*puv)[1]);
}

inline int GetFace(const EGPoint& p) {
  int face = p.LargestAbsComponent();
  if (p[face] < 0) face += 3;
  return face;
}

inline int XYZtoFaceUV(const EGPoint& p, double* pu, double* pv) {
  int face = GetFace(p);
  ValidFaceXYZtoUV(face, p, pu, pv);
  return face;
}

inline int XYZtoFaceUV(const EGPoint& p, R2Point* puv) {
  return XYZtoFaceUV(p, &(*puv)[0], &(*puv)[1]);
}

inline bool FaceXYZtoUV(int face, const EGPoint& p,
                        double* pu, double* pv) {
  if (face < 3) {
...
inline bool FaceXYZtoUV(int face, const EGPoint& p, R2Point* puv) {
  return FaceXYZtoUV(face, p, &(*puv)[0], &(*puv)[1]);
}

inline EGPoint GetUNorm(int face, double u) {
  switch (face) {
    case 0:  return EGPoint( u, -1,  0);
    case 1:  return EGPoint( 1,  u,  0);
    case 2:  return EGPoint( 1,  0,  u);
    case 3:  return EGPoint(-u,  0,  1);
    case 4:  return EGPoint( 0, -u,  1);
    default: return EGPoint( 0, -1, -u);
  }
}

inline EGPoint GetVNorm(int face, double v) {
  switch (face) {
    case 0:  return EGPoint(-v,  0,  1);
    case 1:  return EGPoint( 0, -v,  1);
    case 2:  return EGPoint( 0, -1, -v);
    case 3:  return EGPoint( v, -1,  0);
    case 4:  return EGPoint( 1,  v,  0);
    default: return EGPoint( 1,  0,  v);
  }
}

inline EGPoint GetNorm(int face) {
  return GetUVWAxis(face, 2);
}

inline EGPoint GetUAxis(int face) {
  return GetUVWAxis(face, 0);
}

inline EGPoint GetVAxis(int face) {
  return GetUVWAxis(face, 1);
}

inline EGPoint GetUVWAxis(int face, int axis) {
  const double* p = internal::kFaceUVWAxes[face][axis];
  return EGPoint(p[0], p[1], p[2]);
}

inline int GetUVWFace(int face, int axis, int direction) {
  EG_DCHECK(face >= 0 && face <= 5);
  EG_DCHECK(axis >= 0 && axis <= 2);
  EG_DCHECK(direction >= 0 && direction <= 1);
  return internal::kFaceUVWFaces[face][axis][direction];
}

}  // namespace EG

#endif  // EG_EGCOORDS_H_
