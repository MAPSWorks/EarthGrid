...
using absl::StrCat;
using EG::internal::kSwapMask;
using EG::internal::kInvertMask;
using EG::internal::kPosToIJ;
using EG::internal::kPosToOrientation;
using std::fabs;
using std::max;
using std::min;
using std::vector;
...
const int EGCellID::kFaceBits;
const int EGCellID::kNumFaces;
const int EGCellID::kMaxLevel;
const int EGCellID::kPosBits;
const int EGCellID::kMaxSize;
...
EGCellID EGCellID::advance(int64 steps) const {
    ...
    return EGCellID(id_ + (static_cast<uint64>(steps) << step_shift));
}

int64 EGCellID::distance_from_begin() const {
    const int step_shift = 2 * (kMaxLevel - level()) + 1;
    return id_ >> step_shift;
}

EGCellID EGCellID::advance_wrap(int64 steps) const {
    EG_DCHECK(is_valid());
    if (steps == 0) return *this;
...
EGCellID EGCellID::maximum_tile(const EGCellID limit) const {
    EGCellID id = *this;
    EGCellID start = id.range_min();
    ...
    if (id.range_max() >= limit) {
        ...
        // of EGCellID ranges, this loop usually executes only once.  Also because
...
int EGCellID::GetCommonAncestorLevel(EGCellID other) const {
    // Basically we find the first bit position at which the two EGCellIDs
    // differ and convert that to a level.  The max() below is necessary for the
    // case where one EGCellID is a descendant of the other.
    uint64 bits = max(id() ^ other.id(), max(lsb(), other.lsb()));
    EG_DCHECK_NE(bits, 0);  // Because lsb() is non-zero.
...
EGCellID EGCellID::FromToken(const char* token, size_t length) {
    if (length > 16) return EGCellID::None();
    uint64 id = 0;
...
EGCellID EGCellID::FromToken(const string& token) {
    return FromToken(token.data(), token.size());
}

void EGCellID::Encode(Encoder* const encoder) const {
    encoder->Ensure(sizeof(uint64));  // A single uint64.
    encoder->put64(id_);
}

bool EGCellID::Decode(Decoder* const decoder) {
    if (decoder->avail() < sizeof(uint64)) return false;
    id_ = decoder->get64();
    return true;
}

EGCellID EGCellID::FromFaceIJ(int face, int i, int j) {
...
#undef GET_BITS
    ...
    return EGCellID(n * 2 + 1);
}

EGCellID::EGCellID(const EGPoint& p) {
    double u, v;
    int face = EG::XYZtoFaceUV(p, &u, &v);
    int i = EG::STtoIJ(EG::UVtoST(u));
    int j = EG::STtoIJ(EG::UVtoST(v));
    id_ = FromFaceIJ(face, i, j).id();
}

EGCellID::EGCellID(const EGLatLng& ll)
        : EGCellID(ll.ToPoint()) {
}

int EGCellID::ToFaceIJOrientation(int* pi, int* pj, int* orientation) const {
    // Initialization if not done yet
    MaybeInit();
...
#undef GET_BITS
    ...
    *pj = j;
    ...
    if (orientation != nullptr) {
        ...
        // the kSwapMask bit.
        EG_DCHECK_EQ(0, kPosToOrientation[2]);
        EG_DCHECK_EQ(kSwapMask, kPosToOrientation[0]);
...
EGPoint EGCellID::ToPointRaw() const {
    int si, ti;
    int face = GetCenterSiTi(&si, &ti);
    return EG::FaceSiTitoXYZ(face, si, ti);
}

EGLatLng EGCellID::ToLatLng() const {
    return EGLatLng(ToPointRaw());
}

R2Point EGCellID::GetCenterST() const {
    int si, ti;
    GetCenterSiTi(&si, &ti);
    return R2Point(EG::SiTitoST(si), EG::SiTitoST(ti));
}

R2Point EGCellID::GetCenterUV() const {
    int si, ti;
    GetCenterSiTi(&si, &ti);
    return R2Point(EG::STtoUV(EG::SiTitoST(si)),
                   EG::STtoUV(EG::SiTitoST(ti)));
}

R2Rect EGCellID::IJLevelToBoundUV(int ij[2], int level) {
    R2Rect bound;
    int cell_size = GetSizeIJ(level);
    for (int d = 0; d < 2; ++d) {
        int ij_lo = ij[d] & -cell_size;
        int ij_hi = ij_lo + cell_size;
        bound[d][0] = EG::STtoUV(EG::IJtoSTMin(ij_lo));
        bound[d][1] = EG::STtoUV(EG::IJtoSTMin(ij_hi));
    }
    return bound;
}

R2Rect EGCellID::GetBoundST() const {
    double size = GetSizeST();
    return R2Rect::FromCenterSize(GetCenterST(), R2Point(size, size));
}

R2Rect EGCellID::GetBoundUV() const {
    int ij[2];
...
static double ExpandEndpoint(double u, double max_v, double sin_dist) {
    ...
    // calculation in EGCap::GetRectBound.
...
/* static */
R2Rect EGCellID::ExpandedByDistanceUV(const R2Rect& uv, S1Angle distance) {
...
EGCellID EGCellID::FromFaceIJWrap(int face, int i, int j) {
    ...
    // EG::XYZtoFaceUV() to project back onto the correct face.
    //
    ...
    face = EG::XYZtoFaceUV(EG::FaceUVtoXYZ(face, u, v), &u, &v);
    return FromFaceIJ(face, EG::STtoIJ(0.5*(u+1)), EG::STtoIJ(0.5*(v+1)));
}

inline EGCellID EGCellID::FromFaceIJSame(int face, int i, int j,
                                         bool same_face) {
    if (same_face)
        return EGCellID::FromFaceIJ(face, i, j);
    else
        return EGCellID::FromFaceIJWrap(face, i, j);
}

void EGCellID::GetEdgeNeighbors(EGCellID neighbors[4]) const {
    int i, j;
...
void EGCellID::AppendVertexNeighbors(int level,
                                     vector<EGCellID>* output) const {
    // "level" must be strictly less than this cell's level so that we can
    // determine which vertex this cell is closest to.
    EG_DCHECK_LT(level, this->level());
    int i, j;
    int face = ToFaceIJOrientation(&i, &j, nullptr);
...
void EGCellID::AppendAllNeighbors(int nbr_level,
                                  vector<EGCellID>* output) const {
    EG_DCHECK_GE(nbr_level, level());
    int i, j;
    int face = ToFaceIJOrientation(&i, &j, nullptr);
...
string EGCellID::ToString() const {
    if (!is_valid()) {
...
EGCellID EGCellID::FromDebugString(absl::string_view str) {
    // This function is reasonably efficient, but is only intended for use in
    // tests.
    int level = static_cast<int>(str.size() - 2);
    if (level < 0 || level > EGCellID::kMaxLevel) return EGCellID::None();
    int face = str[0] - '0';
    if (face < 0 || face > 5 || str[1] != '/') return EGCellID::None();
    EGCellID id = EGCellID::FromFace(face);
    for (int i = 2; i < str.size(); ++i) {
        int child_pos = str[i] - '0';
        if (child_pos < 0 || child_pos > 3) return EGCellID::None();
